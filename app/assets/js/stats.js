...
document.addEventListener('DOMContentLoaded', function () {
  var voice   = parseInt(localStorage.getItem('sardab_minutes_voice')   || '0', 10);
  var video   = parseInt(localStorage.getItem('sardab_minutes_video')   || '0', 10);
  var meeting = parseInt(localStorage.getItem('sardab_minutes_meeting') || '0', 10);

  var svEl = document.getElementById('stat-voice');
  var vdEl = document.getElementById('stat-video');
  var mtEl = document.getElementById('stat-meeting');

  if (svEl) svEl.textContent = voice;
  if (vdEl) vdEl.textContent = video;
  if (mtEl) mtEl.textContent = meeting;
});
