...
async function api(method, body) {
  const url = BASE_PATH + '/app/meet/signal.php';
  if (method === 'GET') {
...
async function joinRoom(code, name) {
  ...
  sessionStorage.setItem('sardab-meet-sid', sid);
  sessionStorage.setItem('sardab-meet-name', name.trim());
  sessionStorage.setItem('sardab-meet-room', code);
  sessionStorage.setItem('sardab-meet-creator', res.creator ? '1' : '0');
  window.location.href = BASE_PATH + '/app/meet/lobby/' + code;
}
