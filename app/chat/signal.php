    ...
    foreach ($rooms as $code => $room) {
        if (!empty($room['users'])) {
            foreach ($room['users'] as $sid => $u) {
                // Don't remove creator (first user) until 5 minutes of inactivity
                // Don't remove other users until 2 minutes of inactivity
                $isCreator = ($sid === ($room['creator'] ?? ''));
                $threshold = $isCreator ? 300 : 120;
                if ($now - ($u['ping'] ?? 0) > $threshold) unset($rooms[$code]['users'][$sid]);
            }
        }
        // Only delete room if no users AND no pending signals (give time for late joiners)
...
if ($method === 'GET') {
    ...
    $users = [];
    foreach (($rooms[$room]['users'] ?? []) as $uSid => $uData) { $users[] = ['sid'=>$uSid,'name'=>$uData['name'],'ping'=>$uData['ping']]; }
    echo json_encode(['ok'=>true,'signals'=>[],'users'=>$users,'last'=>$last]);
    exit;
}
...
if ($method === 'DELETE') {
    ...
    if ($room && $sid) {
        ...
        $rooms = json_decode($c, true);
        if (!is_array($rooms)) $rooms = [];
        if (!isset($rooms[$room])) { echo json_encode(['ok'=>true]); exit; }
        unset($rooms[$room]['users'][$sid]);
        $seq = ++$rooms[$room]['seq'];
        $rooms[$room]['signals'][] = ['i'=>$seq, 't'=>'leave', 'f'=>$sid];
        if (empty($rooms[$room]['users'])) {
