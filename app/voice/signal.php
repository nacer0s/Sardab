...
$self = (isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] === 'on' ? 'https' : 'http') . '://' . ($_SERVER['HTTP_HOST'] ?? 'localhost');
if ($origin === $self || preg_match('#^https?://localhost(?::\d+)?$#', $origin)) {
...
if ($method === 'GET') {
    ...
    while (time() - $start < $maxWait) {
        ...
        // Atomic ping update — only update ping, never overwrite signals
...
if ($method === 'POST') {
    ...
    // Single exclusive lock for entire read-modify-write to prevent duplicate seq IDs
    $f = fopen($file, 'c+');
    flock($f, LOCK_EX);
    $c = stream_get_contents($f);
