...
if (preg_match('#\.(css|js|svg|png|jpg|jpeg|webp|ico|json|woff2?|ttf|otf|wasm|mp4|webm|ogg|mp3)(\?.*)?$#', $uri)) {
  ...
  $clean = str_replace('/', DIRECTORY_SEPARATOR, $relUri);
  $clean = preg_replace('#[\\/]\.\.(?:[\\/]|$)#', '', $clean);
  $clean = ltrim($clean, '\\/');
  $baseDir = realpath(dirname(__DIR__, 2)) ?: __DIR__;
  $file = $baseDir . DIRECTORY_SEPARATOR . $clean;
  $real = realpath($file);
  if ($real && strpos($real, $baseDir . DIRECTORY_SEPARATOR) === 0 && is_file($real)) {
