...
async function api(method, body) {
  const url = BASE_PATH + '/app/video/signal.php';
  if (method === 'GET') {
...
async function joinRoom(code, name) {
  ...
  sessionStorage.setItem('sardab-video-sid', sid);
  sessionStorage.setItem('sardab-video-name', name.trim());
  sessionStorage.setItem('sardab-video-room', code);
  sessionStorage.setItem('sardab-video-creator', res.creator ? '1' : '0');
  window.location.href = BASE_PATH + '/app/video/lobby/' + code;
}
